...
import { toast } from "react-hot-toast"; // use named import
...
function AddLecture() {
  ...
  useEffect(() => {
    // prefer navigation state, fallback to :id param
    const idFromState = courseDetails?._id;
    const idFromParam = params?.id;
    const resolvedId = idFromState || idFromParam;
    if (!resolvedId) {
      navigate("/courses");
      return;
    }
    setUserInput((s) => ({ ...s, id: resolvedId }));
  }, [courseDetails, params, navigate]);
    ...
    async function onFormSubmit(e) {
        ...
        const response = await dispatch(addLectures(userInput));
